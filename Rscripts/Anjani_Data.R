...
a42<-read.csv("A42_mutants1.3.csv", stringsAsFactors = F)
...
ggsave(filename="Mean.byMutatnts.byAntibio.1.3_2.pdf",width =12, height =8)
...
ggsave(filename="Mean.byProperty1.3.pdf",width =7, height =5)

########
## 13ug

a42<-read.csv("A42_mutants13.csv", stringsAsFactors = F)
a42$Mutants<-gsub(" .*","",a42$Mutants)
a42[1, 3:4]<-"Control"
a42[5, 3:4]<-"WT"
#extract the AA and Property info
...
plot_grid(title, title,Plot_1,Plot_2,Plot_3,Plot_4, nrow = 3, ncol=2, labels = c('','',antibio),label_size = 12,
          label_y=1.05, rel_heights = c(0.2, 1,1))
ggsave(filename="Mean.byMutatnts.byAntibio.13.pdf",width =12, height =8)
...
ggplot(propSum)+
    ...
    theme(axis.text.x = element_text(angle = 90, size=12, hjust=1),
          ...
          panel.grid.major.x = element_blank())+
    scale_fill_manual(values=col4)
ggsave(filename="Mean.byProperty13.pdf",width =7, height =5)
...
#col4<-qualitative_hcl(4, palette="Set2")
ggplot(propSum2)+
    geom_bar(aes(x=Property, y=Mean ), stat="identity",width=0.8, fill=col4[4])+
    ...
    theme(axis.text.x = element_text(angle = 90, size=12, hjust=1, color=1),
          legend.title = element_blank(),
          panel.grid.major.x = element_blank())
ggsave(filename="Mean.byProperty13_grouped.pdf",width =6, height =5)
...
wilcox.test(aa2$value[aa$Properties.of.Amino.Acid=="WT"],aa2$value[aa$Properties.of.Amino.Acid=="Cyclic"], alternative ="less", paired=FALSE )
#W = 0, p-value = 0.0000261
write.csv(wilcox.results, "Wilcox.results.compareToWT.csv")
